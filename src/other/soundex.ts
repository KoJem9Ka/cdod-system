...
const wordFunc = function ( str: string, iscyr1: boolean ) {
  ...
  while ( i < l ) {
    ...
    if ( i == 0 ) {
      code = last[0][0]
      // @ts-ignore
    } else if ( !str[i + j] || codes[str[i + j]][0][0] != 0 ) {
...
const calc     = function ( str1: string ): string | null {
  ...
  for ( let i = 0, l = matches.length; i < l; i++ ) {
    ...
    if ( cache[word] ) {
      // @ts-ignore
      matches[i] = cache[word]
    } else {
      // @ts-ignore
      matches[i] = cache[word] = wordFunc( word, iscyr )
    }
  }
  return matches.join( '' )
}
const translit = function ( str: string, matches: string[] ) {
  for ( let i = 0, l = matches.length; i < l; i++ ) {
