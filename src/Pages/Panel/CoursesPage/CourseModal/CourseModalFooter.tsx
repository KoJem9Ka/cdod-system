...
const CourseModalFooter: React.FC = () => {
  ...
  const { isEdit, isModified, toggleEdit } = useCourseForm()
    ...
    <Footer>
      {
        isEdit
          ? <>
            <CourseButton onClick={() => toggleEdit()}>Отмена</CourseButton>
            <CourseButton disabled={!isModified()}>Сохранить</CourseButton>
          </>
          : <>
            <CourseButton onClick={() => toggleEdit()}>Изменить</CourseButton>
          </>
      }
    </Footer>
  )
